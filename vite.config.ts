    ...
    rollupOptions: {
      output: {
        ...
        assetFileNames: (assetInfo) => {
          const cssEntryNames = new Set(['index.css', 'tokens.css', 'fonts.css'])
          const isCssEntry = (assetInfo.names ?? [assetInfo.name]).some((name) =>
            name ? cssEntryNames.has(name) : false,
          )
          return isCssEntry ? '[name][extname]' : 'assets/[name]-[hash][extname]'
        },
      },
    },
  },
