...
- `pnpm check:css-tree-shaking` (`scripts/check-css-tree-shaking.ts`, runs in CI in the `check` job) — the CSS counterpart to `check:bundle-size`, which measures JS only. Asserts each component's compiled CSS contains only its own rules and that `interactions.module.css`'s shared rules ship exactly once — first structurally against `dist/`, then against a scratch consumer built on an `npm pack`ed tarball installed into a `mktemp -d` directory. Losing `preserveModules` (every component's CSS collapses back into one eager `dist/index.css`) or reintroducing a `composes:` against a shared CSS module (six duplicated copies) both otherwise produce a green build. Pass `--skip-consumer` to run only the fast structural half locally.
