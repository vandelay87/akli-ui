...
- **Publishing is two merges, not one — do not conflate this with the Storybook deploy below, which fires on every merge unconditionally.** On merge to `main`, the Changesets GitHub Action checks for pending changeset files: if any exist, it opens/updates a "Version Packages" PR (bumps `package.json`, writes the changelog) and **publishes nothing yet**. Only merging *that* Version Packages PR — a second, separate merge to `main` — triggers the actual `npm publish`. A regular component PR merged without a changeset file queues nothing at all; it sits in `main` unversioned until someone adds one.
...
- **Deploys unconditionally on every merge to `main`** — unlike the npm publish flow (see Versioning & release), this isn't gated behind Changesets or a version bump. Storybook reflects whatever's currently in `main`, not a published package version, so it redeploys every time regardless of whether that merge included a changeset
